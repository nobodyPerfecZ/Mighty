...
class DummySACModel(nn.Module):
    ...
    def policy_log_prob(self, z, mean, log_std):
        ...
        log_prob = -0.5 * (((z - mean) / log_std.exp()).pow(2) + 2 * log_std + np.log(2 * np.pi))
...
class TestSACUpdate:
    ...
    def test_target_network_updates(self):
        ...
        for p_target, p_main, p_old_target in zip(
            ...
            assert torch.allclose(p_target, expected, atol=1e-5), "Target update should follow polyak averaging"
    ...
    def test_td_error_calculation(self):
        ...
        # Check shapes
        assert td_error1.shape == (batch.batch_size, 1), f"TD error1 shape: {td_error1.shape}"
        assert td_error2.shape == (batch.batch_size, 1), f"TD error2 shape: {td_error2.shape}"
    ...
    def test_different_tau_values(self):
        ...
        initial_target_large = [p.clone() for p in model_large.target_q_net1.parameters()]
        update_large.update(batch)
        
        total_change_large = sum(
            torch.norm(p1 - p2).item()
            for p1, p2 in zip(initial_target_large, model_large.target_q_net1.parameters())
        )
        
        assert total_change_large > total_change, "Larger tau should cause bigger target network changes"
    ...
    def test_zero_rewards_batch(self):
        ...
        for metric_name, metric_value in metrics.items():
            assert np.isfinite(metric_value), f"Metric {metric_name} should be finite with zero rewards"
    ...
    def test_all_done_batch(self):
        ...
        for metric_name, metric_value in metrics.items():
            assert np.isfinite(metric_value), f"Metric {metric_name} should be finite with all done"
    ...
    def test_gradient_flow(self):
        ...
        assert change_ratio > 0.1, f"Only {change_ratio:.2%} of parameters changed, gradient flow might be broken. Changes: {param_changes}"
