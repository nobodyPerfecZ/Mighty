...
class SACEtaUpdate(SACUpdate):
    """SAC update with a learnable risk parameter eta (port of rejax's SACEta).

    The Q-networks regress onto an entropic risk transform of the TD target,
    v_entropic = (1/eta) * log(exp(eta * target)), and eta itself is adapted
    with the loss eta * (target_risk_ratio - (target - mean(target))).
    """
    ...
    def __init__(
        ...
        self.eta_optimizer = optim.Adam([self.log_eta], lr=eta_lr or q_lr)

    def update(self, batch: TransitionBatch) -> Dict:
        """Perform an update of the SACEta model using a batch of experience."""
        self.update_step += 1
        ...
        with torch.no_grad():
            ...
            # Entropic risk transform of the target (as in rejax's SACEta).
        ...
        q_loss1 = F.mse_loss(q1, v_entropic)
        q_loss2 = F.mse_loss(q2, v_entropic)
        ...
        # --- Eta update ---
        eta = self.log_eta.exp()
        target_bar = q_target.mean()
        eta_loss = (eta * (self.target_risk_ratio - (q_target - target_bar))).mean()
            ...
            for _ in range(self.policy_frequency):
                ...
                if self.auto_alpha:
                    ...
                    with torch.no_grad():
                        ...
                        logp_alpha = self.model.policy_log_prob(
                            z_alpha, mean_alpha, log_std_alpha
                        )

                    alpha_loss = -(
                        self.log_alpha.exp()
                        * (logp_alpha.detach() + self.target_entropy)
                    ).mean()
        ...
        return {
            ...
            "Update/eta_loss": eta_loss.item(),
            "Update/eta_value": self.log_eta.exp().item(),
